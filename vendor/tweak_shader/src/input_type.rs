...
impl<'a> MutInput<'a> {
    ...
    pub fn as_point3(&mut self) -> Option<&mut BoundedInput<[f32; 3]>> {
        extract!(self.inner, InputType::Point3(i) => i)
    }
    ...
    pub fn copy_into(&mut self, other: &mut Self) -> bool {
        match (&mut self.inner, &mut other.inner) {
            ...
            (InputType::Point3(s), InputType::Point3(o)) => {
                if s.current <= o.max && s.current >= o.min && s.default == o.default {
                    o.current = s.current;
                    true
                } else {
...
impl FromRanges<[f32; 3]> for BoundedInput<[f32; 3]> {
    fn from_ranges(
        min: Option<[f32; 3]>,
        max: Option<[f32; 3]>,
        default: Option<[f32; 3]>,
    ) -> Self {
        let default_f32 = default.unwrap_or([0.0, 0.0, 0.0]);
        let range_x = default_f32[0].abs().max(1.0) * 10.0;
        let range_y = default_f32[1].abs().max(1.0) * 10.0;
        let range_z = default_f32[2].abs().max(1.0) * 10.0;

        let min_v = min.unwrap_or([-range_x, -range_y, -range_z]);
        let max_v = max.unwrap_or([range_x, range_y, range_z]);
...
pub enum InputType {
    ...
    /// A vec3 representing a 3d point or vector.
    /// ```text
    /// #pragma input(point3, name="pos", max=[1.0, 1.0, 1.0], min=[-1.0, -1.0, -1.0], default=[0.0, 0.0, 0.0])
    /// ```
    Point3(BoundedInput<[f32; 3]>),
...
impl TryAsMut<[f32; 3]> for InputType {
    fn try_as_mut(&mut self) -> Option<&mut [f32; 3]> {
        extract!(self, InputType::Point3(p) => &mut p.current)
    }
}
...
impl InputType {
    ...
    pub fn as_bytes_mut(&mut self) -> &mut [u8] {
        match self {
            ...
            Self::Point3(v) => bytes_of_mut(&mut v.current),
    ...
    /// Returns the std140 alignment requirement of this input's
    /// uniform representation. Used when packing the CPU-side backing
    /// buffer so offsets match naga's std140 layout.
    pub fn std140_alignment(&self) -> usize {
        match self {
            Self::Color(_) | Self::Point3(_) => 16,
            Self::Point(_) => 8,
            // Conservative: unsupported blobs (matrices, arrays) align to 16.
            Self::RawBytes(v) => {
                if v.inner.len() >= 16 {
                    16
                } else {
    ...
    pub fn as_bytes(&self) -> &[u8] {
        match self {
            ...
            Self::Point3(v) => bytes_of(&v.current),
...
impl From<&InputType> for InputVariant {
    fn from(variant: &InputType) -> InputVariant {
        match variant {
            ...
            InputType::Point3(_) => InputVariant::Point3,
