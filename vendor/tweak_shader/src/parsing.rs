...
impl FromStr for InputEntry {
    ...
    fn from_str(input: &str) -> Result<Self, Self::Err> {
        ...
        match type_id.as_str() {
            ...
            "point3" => {
                let (name, point) = create_input(rest).map_err(bail)?;
                Ok(InputEntry(name, InputType::Point3(point)))
            }
            "bool" => {
    ...
    fn try_from(value: QVal) -> Result<Self, Self::Error> {
        if let QVal::List(list) = value {
            if list.len() == 3 {
                ...
                Err(Error::UnexpectedType(
                    "Wrong number of elements in point3 list".to_string(),
                ))
            }
        } else {
            Err(Error::UnexpectedType(
                "Expected type Point3 ([f32 ; 3])".to_string(),
            ))
        }
    }
}
